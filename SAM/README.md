# Execution instructions to successfully run the SAM Gan for aging in our project 

## Pre requisits

All of the files required for the StyleGAN part of the project are found in the SAM folder

The first thing needed is to install the pretrained StyleGAN model for ffhq, and to put it in a folder called pretrained_models.

The next thing needed is to run pip install -r SAM/requirements.txt (or just requirements.txt if in the SAM folder)

## running the models

to run, the following bash command must be run from a terminal in the SAM folder: 

source /PATH_TO_VENV/.venv/bin/activate && python scripts/inference_side_by_side.py --exp_dir=PATH_TO_OUTPUT_DIR --checkpoint_path=pretrained_models/sam_ffhq_aging.pt --data_path=PATH_TO_IMAGES_TO_AGE --test_batch_size=4 --test_workers=4 --target_age=0,10,20,30,40,50,60,70,80 

target age can be a list of any size of ints, the results start to look very incoherent after about 150. The example will generate a side by side comparison with the original image, and all the target age resulting images.


# Everything below is the original readme from the git of this project : https://github.com/yuval-alaluf/SAM
